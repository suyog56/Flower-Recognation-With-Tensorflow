...
# Function to remove RandomFlip layer from model configuration
def remove_random_flip(config):
    for layer in config['config']['layers']:
        if layer['class_name'] == 'RandomFlip':
            config['config']['layers'].remove(layer)
            break
    return config

# Load model with custom object handling
try:
    model = load_model(local_model_path, custom_objects={'remove_random_flip': remove_random_flip})
